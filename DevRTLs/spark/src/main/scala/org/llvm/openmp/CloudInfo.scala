...
class CloudInfo(args: Array[String]) {
  ...
  filesystem match {
    case "S3" =>
      ...
      fsConf.set("fs.s3n.awsAccessKeyId", args(4))
      fsConf.set("fs.s3n.awsSecretAccessKey", args(5))

      //sc.hadoopConfiguration.set("fs.defaultFS", uri)
    case "HDFS" =>
      System.setProperty("HADOOP_USER_NAME", username)
      fsConf = sc.hadoopConfiguration
    case _ => throw new RuntimeException(filesystem + " is not a supported file system.")
  }

  val sqlContext = new SQLContext(sc)

  val fs = FileSystem.get(URI.create(uri), fsConf)
